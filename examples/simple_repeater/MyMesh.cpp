...
bool MyMesh::allowPacketForward(const mesh::Packet *packet) {
  ...
  if (packet->isRouteFlood()) {
    ...
    if (type == PAYLOAD_TYPE_GRP_TXT || type == PAYLOAD_TYPE_GRP_DATA) {
      if (packet->getPathHashCount() >= getEffectiveFloodMax(busy_tracker.busy,
      ...
      if (packet->getPathHashCount() >= getEffectiveFloodMax(busy_tracker.busy,
        ...
        return false;
    } else if (packet->getPathHashCount() >= _prefs.flood_max) {
      return false;
    }
  }
...
uint32_t MyMesh::getRetransmitDelay(const mesh::Packet *packet) {
  ...
  float scale = 1.0f + busy_tracker.busy * 2.0f;  // busy=0: unchanged, busy=1: 3× wider window
...
void MyMesh::loop() {
  ...
  busy_tracker.update(now, getTotalAirTime(), getReceiveAirTime(),
                      _mgr->getOutboundCount(0xFFFFFFFF), PACKET_POOL_SIZE,
